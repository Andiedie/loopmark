...
Loopmark is a cloud human-input handoff for AI agents. It lets an agent create a small encrypted question session, send a public fill URL to a human, and continue from the Answer Text the human pastes back.
...
- Changing the CLI, Worker API, browser fill page, Answer Text, secret handling, or bundled agent skill.
...
- Answer Text rendering: `src/shared/answer-text.ts`.
...
- Human: the person opening the fill URL and pasting copied Answer Text back.
...
- Answer Text: the plain-text answer transport pasted back to the agent.
- Secret bundle: encrypted browser upload containing only secret answers that were omitted from Answer Text.

## Verification

- Protocol or skill docs: `pnpm exec vitest run tests/skill.test.ts --coverage=false`.
- Schema or answer behavior: targeted tests under `tests/schema.test.ts`, `tests/submission.test.ts`, and `tests/answer-text.test.ts`.

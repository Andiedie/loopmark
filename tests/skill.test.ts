...
describe("bundled Loopmark skill", () => {
  ...
  it("keeps a strict human-input boundary and avoids polling", () => {
    ...
    expect(skill).toContain("Codex Desktop");
    expect(skill).toContain("in-app browser");
    expect(skill).toContain("already available");
    expect(skill).toContain("Outside Codex Desktop, send the URL normally");
    expect(skill).toContain("do not use OS browser commands, Chrome automation, or Playwright");
    expect(skill).toContain("Do not search for tools, install plugins");
    expect(skill).toContain("Browser opening is only a convenience");
    expect(skill).toContain("always include `fillUrl` as a fallback");
    expect(skill).toContain("Keep `receiptFile` local");
    expect(skill).toContain("inspect the page, or take screenshots");
    expect(skill).toContain("Do not rerun create, poll, read browser state, or attempt automatic continuation");
    expect(skill).toContain("Fallback link: `fillUrl`");
    expect(skill).toContain("send me the Answer Text");
    expect(skill).not.toContain("答案文本");
  ...
  it("keeps custom base URL in the protocol and deployment details in the operations runbook", () => {
    ...
    expect(protocol).toContain("In Codex Desktop");
    expect(protocol).toContain("Outside Codex Desktop, send the URL normally");
    expect(protocol).toContain("Do not search for tools or install plugins");
    expect(protocol).toContain("The Answer Text flow is unchanged");
    expect(protocol).toContain("Always include the `fillUrl` as a fallback");
    expect(protocol).toContain("Do not inspect the page or take screenshots");
    expect(protocol).toContain("ask the human to click Copy answers and send Answer Text");
    expect(protocol).toContain("agents must end the turn rather than poll");
